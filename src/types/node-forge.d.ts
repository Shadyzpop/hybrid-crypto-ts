...
declare module "node-forge" {
    ...
    namespace random {
        ...
        // prng context
        function generate(count: number, callback: (err?: string, bytes?: string) => void): string;
        function generateSync(count: number): string;
        function defaultSeedFile(needed: number): string;
        function collect(bytes: string): void;
        function collectInt(integer: number, nBytes: number): void;
        // web only
        function registerWorker(worker: Window | Document): void;
    }
